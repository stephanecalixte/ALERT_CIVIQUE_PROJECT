...
// Supprimer la vidéo associée à un livestreamId
const deleteVideoByLivestreamId = async (req, res) => {
  try {
    const livestreamId = parseInt(req.params.livestreamId);
    if (isNaN(livestreamId)) {
      return res.status(400).json({ error: 'livestreamId invalide' });
    }

    const video = await Video.findOne({ livestreamId });
    if (!video) {
      return res.status(404).json({ message: 'Aucune vidéo associée à ce livestream' });
    }
    ...
    await Video.deleteOne({ livestreamId });
    console.log(`🗑️ Deleted video document for livestreamId: ${livestreamId}`);

    res.json({ success: true, message: 'Vidéo supprimée avec succès' });
  } catch (error) {
    console.error('Error deleting video by livestreamId:', error);
    res.status(500).json({ error: 'Erreur lors de la suppression de la vidéo' });
  }
};
