...
const { handleSocketConnection, getConnectedUsersCount } = require('./socket/socketHandlers');
const { errorHandler, notFound } = require('./middleware/errorHandler');
const { getLocalIPs, formatBytes } = require('./utils/helpers');
...
app.get('/health', (req, res) => {
  ...
  res.json({
    ...
    uploadDir: uploadDirExists ? 'ready' : 'missing',
    videosCount: videosCount,
    uploadDirSize: formatBytes(uploadDirSize),
    memoryUsage: process.memoryUsage()
  });
});
...
// Middleware pour passer io aux routes (pour l'alerte)
app.use((req, res, next) => {
  req.io = io;
  next();
});

// Middleware pour capturer l'alerte et l'émettre via socket
const originalSendAlert = require('./controllers/messageController').sendAlert;
app.post('/api/alert', (req, res, next) => {
  const originalJson = res.json;
  res.json = function(data) {
    if (res.locals.alertData) {
      io.emit('alertMessage', res.locals.alertData);
    }
    originalJson.call(this, data);
  };
  next();
}, require('./controllers/messageController').sendAlert);

// Gestion des erreurs
app.use(notFound);
app.use(errorHandler);

// Exposer io pour les routes (notifications livestream)
app.set('io', io);
